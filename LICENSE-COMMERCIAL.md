# SaaS Contract Auditor - Commercial License

## Overview

SaaS Contract Auditor is dual-licensed. The source code is available under the **GNU Affero General Public License v3.0 (AGPL-3.0)** for open source use. For individuals, startups, and enterprises that prefer not to comply with AGPL-3.0 obligations, commercial licenses are available under the terms described below.

A commercial license grants you a perpetual, non-exclusive right to use SaaS Contract Auditor in proprietary and closed-source applications without the AGPL-3.0 copyleft requirements.

---

## License Tiers

### Personal / Startup License - $299 (one-time)

A perpetual license for use by a single company or individual in a single product or internal tool.

- Use in one commercial product or internal application
- Covers your entire team within that single company (no per-seat limits)
- Perpetual right to use the version available at time of purchase, including all updates within that major version line
- No recurring fees

### Enterprise License - $999 (one-time)

A perpetual license for organizations that need broader usage rights.

- Use across unlimited products and internal applications
- Resale and redistribution rights included (e.g., embedding in a product you sell to your customers)
- Covers your entire organization, all subsidiaries, and all teams
- Perpetual right to use the version available at time of purchase, including all updates within that major version line
- No recurring fees

---

## What's Included

Both commercial license tiers include:

- **Perpetual usage rights**, your license never expires
- **All updates** released within the major version line at time of purchase
- **Full source code access**, same codebase as the open source version
- **Freedom from AGPL-3.0 copyleft**, no obligation to open-source your own application
- **No recurring fees**, pay once, use forever

---

## Restrictions

The following restrictions apply to both commercial license tiers unless otherwise noted:

1. **No sub-licensing.** You may not grant licenses to third parties to use SaaS Contract Auditor independently, except where redistribution rights are explicitly included (Enterprise License).
2. **No misrepresentation of origin.** You may not claim authorship of SaaS Contract Auditor itself, though you may integrate it into your own product without attribution requirements.
3. **One license per entity.** The Personal/Startup License covers one company. If you operate multiple independent companies, each requires its own license. The Enterprise License covers the purchasing organization and its subsidiaries.
4. **No warranty.** The software is provided "as is" without warranty of any kind. See the full disclaimer below.

---

## How to Purchase

To purchase a commercial license, contact us directly:

**Email:** [a.saxena.email@gmail.com](mailto:a.saxena.email@gmail.com)

Please include:

- Your name or company name
- Which license tier you are interested in (Personal/Startup or Enterprise)
- A brief description of your intended use

You will receive a signed license agreement and payment instructions. The license is effective upon receipt of payment.

---

## FAQ

### Can I use the open source version commercially?

Yes. The AGPL-3.0 license permits commercial use. However, AGPL-3.0 requires that if you run a modified version of the software as a network service (e.g., a SaaS product), you must make the complete source code of your entire application available to its users under AGPL-3.0. If you prefer to keep your application proprietary, a commercial license removes this requirement.

### What if my team grows?

The Personal/Startup License covers your entire company, not individual seats. Whether you have 2 developers or 200, a single Personal/Startup License covers all of them for use in one product. If you need to use SaaS Contract Auditor across multiple products, consider the Enterprise License.

### Do I get support?

- **Open source (AGPL-3.0) users:** Community support via GitHub issues and discussions.
- **Commercial licensees (both tiers):** Email support at [a.saxena.email@gmail.com](mailto:a.saxena.email@gmail.com). We aim to respond within 2 business days.

### Can I upgrade from Personal/Startup to Enterprise later?

Yes. Contact us and you will only pay the difference between the two tiers.

### What happens if a new major version is released?

Your license covers all updates within the major version line you purchased. Access to a new major version requires a separate purchase at a discounted upgrade price. You may continue using your licensed version indefinitely.

---

## Disclaimer

THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NONINFRINGEMENT. IN NO EVENT SHALL THE AUTHORS OR COPYRIGHT HOLDERS BE LIABLE FOR ANY CLAIM, DAMAGES, OR OTHER LIABILITY, WHETHER IN AN ACTION OF CONTRACT, TORT, OR OTHERWISE, ARISING FROM, OUT OF, OR IN CONNECTION WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS IN THE SOFTWARE.

---

Copyright (c) 2026 SaaS Contract Auditor. All rights reserved.
