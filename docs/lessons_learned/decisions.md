...
- Structured tracing via `src/tracing.py`: JSON logs to stderr with request IDs, timing, and context. Uses Python `logging` module, captured by Docker container runtime. Set `LOG_LEVEL=DEBUG` for verbose output. Includes in-memory metrics counters (reports generated, errors, retries, durations) exposed via `get_metrics()`
- Evaluation harness in `evaluation/`: 15 contract cases with expected classifications. `run_eval.py` runs each through `analyze_account()` and reports accuracy + quality scores (section coverage, metric coverage, ARR at Risk presence). TYPE_EQUIVALENTS dict handles acceptable near-misses (e.g. "upsell proposition" and "at capacity" are close enough). Supports `--mock` flag for CI (uses pre-recorded fixtures from `evaluation/fixtures/`)
- CI workflow triggers on `master` branch (not `main`). The default branch is `master`
- Pydantic `ReportMetadata` model validates LLM output metadata (proposition_type, success_percent 0-100, priority_score 1-10, etc.). On parse failure, falls back to `with_structured_output()` for a second extraction attempt. Replaces silent defaults that hid bad LLM output
- `src/resilience.py` provides: (1) `invoke_with_retry()` with exponential backoff for transient LLM failures (rate limits, timeouts, connection errors), (2) `llm_semaphore` (asyncio.Semaphore, default 5, configurable via `MAX_CONCURRENT_LLM` env var) to prevent rate-limit storms during fan-out, (3) `get_http_client()` shared httpx.AsyncClient with connection pooling and 2 transport-level retries
- Report section validation: after LLM Pass 1, checks for 9 required section headers. If missing, does one focused re-prompt to add only the missing sections. Non-blocking numeric consistency check verifies input metric values appear in report text (logs warning if >50% absent)
- Health check endpoint at `/api/health`: checks DB (SELECT 1) and agent (HTTP ping to LangGraph). Returns 200/503 with component status. Docker Compose uses it for container healthcheck
- Graceful degradation: CopilotKit route handler wrapped in try/catch, returns 503 with user-friendly message when agent is unreachable
- LLM evaluator node replaces deterministic string-matching checks (section headers, numeric grep). Uses `with_structured_output(ReportEvaluation)` to score reports against a rubric. On "fail", re-analyzes with feedback (max 1 retry). More reliable than string matching for catching fabricated data, wrong classifications, and hallucinated deal references
- Numeric consistency moved from deterministic string-matching to LLM-based evaluation. The evaluator checks whether input metric values appear correctly in the report, catching cases where the LLM reformats or rounds numbers that string matching would miss
- Langfuse integration is additive: `get_langfuse_config()` returns None when keys are not set, and all `invoke_with_retry()` calls accept an optional `config` parameter. No behavior change without Langfuse keys
- When mocking ChatOpenAI in tests, use MagicMock (not AsyncMock) for the model object, with `model.ainvoke = AsyncMock(...)`. AsyncMock makes `with_structured_output()` return a coroutine instead of the mock, breaking the evaluator call chain
- Webhook ingest uses HMAC-SHA256 (same pattern as GitHub/Stripe/Slack). Single atomic POST endpoint for full account payload (account + metrics + budget + documents) keeps caller integration simple vs. separate endpoints per resource
- `account_documents` table stores qualitative data (contracts, usage logs, CS notes) separately from numeric `account_usage_metrics`. Documents flow into agent via `getAccountSummaries()` automatically
- Future-proofing seams added in Plan 029: (1) `tenant_id` nullable column on accounts (no queries use it yet), (2) pluggable auth middleware pattern (`withWebhookAuth` wrapper), (3) `audit_events` table (write-only, no UI). These are cheap to add now, expensive to retrofit
- Auth middleware separates concerns: `webhook-auth.ts` is a pure HMAC verification function, `auth-middleware.ts` is the Next.js integration layer (reads body, verifies, returns 401/403). New auth strategies (session, OIDC) become new wrapper functions in auth-middleware.ts
- `writeAuditEvent()` is fire-and-forget (try/catch with console.error). Audit logging never blocks or fails the webhook request
